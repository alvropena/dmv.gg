...
export default function Features() {
  ...
  const FeatureCard = ({ feature }: { feature: (typeof features)[0] }) => (
    <div className="bg-white/50 rounded-3xl p-6 flex flex-col items-center text-center h-full">
      <div className="w-full h-[300px] rounded-2xl bg-[#fff] backdrop-blur-sm p-6 flex items-center justify-center mb-6 relative overflow-hidden">
        <Image
          src={feature.image}
          alt={feature.title}
          fill
      ...
      <p className="text-[#3F3500]/80">{feature.description}</p>
    </div>
  );
  ...
  return (
    <section
      id="features"
      className="w-full py-12 md:py-12 lg:py-52 bg-[#FFF25F]"
    >
            ...
            {features.map((_, index) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-colors ${
                  index === current ? "bg-[#3F3500]" : "bg-[#3F3500]/20"
                }`}
