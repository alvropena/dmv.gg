                      ...
                      <div className="grid grid-cols-1 gap-2 mt-2 border-t pt-2">
                          ...
                          <div className="text-sm">
                            ...
                            <span className="font-medium">
                              {(() => {
                                ...
                                const completedCount = session.answers && Array.isArray(session.answers)
                                  ? session.answers.filter(a => a.selectedAnswer).length
                                  : session.score;
                                
                                // Calculate progress percentage
                                const progressPercent = Math.round((completedCount / session.totalQuestions) * 100);
                                
                                // Calculate remaining questions
                                const remainingCount = session.totalQuestions - completedCount;
                                
                                return (
                                  <>
                                    <span className="text-primary">{progressPercent}%</span>
                                    {" "}
                                    ({completedCount} completed, {remainingCount} left)
                                  </>
                                );
                              })()}
                            </span>
                          ...
                          <div className="text-sm mt-1">
                            ...
                            <span className="font-medium">
                              {(() => {
                                ...
                                const answeredQuestions = session.answers && Array.isArray(session.answers)
                                  ? session.answers
                                      .filter(a => a.selectedAnswer)
                                      .sort((a, b) => 
                                        new Date((b.answeredAt || new Date(0)) as Date).getTime() - 
                                        new Date((a.answeredAt || new Date(0)) as Date).getTime()
                                      )
                                  : [];
                                ...
                                if (answeredQuestions.length > 0) {
                                  ...
                                  const questionNumber = session.answers 
                                    ? session.answers.findIndex(a => a.questionId === lastAnswer.questionId) + 1
                                    : 1;
                                  
                                  // Access the question title if available
                                  if (lastAnswer.question && lastAnswer.question.title) {
                        ...
                        {session.status === "completed" && (
                          <div className="text-sm mt-1">
                            ...
                            <span className="font-medium">{session.score}/{session.totalQuestions} ({Math.round((session.score / session.totalQuestions) * 100)}%)</span>
                          </div>
                        )}
                      </div>
                    </div>
